...
//单链表倒置创建新单链表
Linklist* linklist_reorder_new(Linklist *head){
	if(NULL == head){
		return NULL;
	}
	Linklist *head_ret = linklist_create();
	Linklist *p = head->next;
	Linklist *t_front = NULL;
	while(p != NULL){
	...
	head_ret->next = t_front;
	return head_ret;
}
void linklist_reorder(Linklist *head){
	if(NULL == head){
		return ;
	}
	if(head->next == NULL){
